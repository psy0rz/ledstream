...
Compiles the real firmware QOIS decoder (`main/qois.hpp`) on the host (only
`FastLED.h`/`leds.hpp` are stubbed), feeds it a recorded ledder stream byte-by-byte the
way `Ledstreamer::process()` does, and checks the led buffer bit-exactly against the raw
frames after every frame — including flash-replay style loop restarts (`resetStream()` +
cleared leds) and the persistent-across-frames color index / `QOIS_OP_PREVFRAME`
behavior.
